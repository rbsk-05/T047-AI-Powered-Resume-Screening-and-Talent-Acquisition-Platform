...
class RankingService:
    """Rank and compare candidates using the shared transparent scoring model."""
    ...
    def compare(self, request: RankingRequest) -> ComparisonResult:
        return ComparisonResult(candidates=[
            CandidateComparison(candidate_name=name, overall_match_score=match.overall_match_score, matched_skills=match.matched_skills, missing_required_skills=match.missing_required_skills, missing_preferred_skills=match.missing_preferred_skills)
